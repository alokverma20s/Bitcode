  ...
  if (options.length > 0 && type === "select") {
    return (
      <div className="relative w-full">
        ...
        <label
          htmlFor={id}
          className={`absolute top-4 left-2 py-0 ${disabled ? "bg-white -top-2.5 text-xs lg:text-xs" : ""} lg:text-[16px] text-[14px] select-none px-1.5 text-gray-400 cursor-text peer-focus:text-xs peer-focus:-top-2.5 peer-focus:bg-white transition-all duration-300 peer-focus:text-primary-600 peer-valid:bg-white peer-valid:-top-2.5 peer-valid:text-xs whitespace-nowrap z-10`}
        >
  ...
  return (
    <div className="relative w-full">
      <input
        ...
        name={name}
        onChange={onChange}
        className={`no-spinners py-2 px-4 rounded-xl w-full h-14 outline outline-1 outline-gray-300 focus:outline-none focus:border-primary-500 focus:border-2 transition-colors duration-300 peer ${disabled ? "cursor-not-allowed" : ""}`}
      />
      <label
        htmlFor={id}
        className={`absolute top-4 left-2 py-0 ${disabled ? "bg-white -top-2.5 text-xs lg:text-xs" : ""} lg:text-[16px] text-[14px] select-none px-1.5 text-gray-400 cursor-text peer-focus:text-xs peer-focus:-top-2.5 peer-focus:bg-white transition-all duration-300 peer-focus:text-primary-600 peer-valid:bg-white peer-valid:-top-2.5 peer-valid:text-xs whitespace-nowrap z-10`}
